...
El objetivo general (Sección~\ref{subsec:objetivo_general}) —diseñar y desarrollar un sistema de cálculo de rutas seguras para peatones en Madrid, integrando iluminación, vulnerabilidad territorial y accidentalidad mediante enrutamiento sobre grafos ponderados— se considera \textbf{alcanzado}: el Capítulo~\ref{cap:pruebas} ha demostrado, sobre casos concretos, que el sistema resultante ofrece efectivamente una alternativa medible en seguridad a la ruta más corta convencional. La Tabla~\ref{tab:cumplimiento_oe} detalla el grado de cumplimiento de cada uno de los seis objetivos específicos (Sección~\ref{subsec:objetivos_especificos}).
...
Cada limitación de la Sección~\ref{sec:limitaciones_sistema} define una línea de trabajo futuro directa:
