...
El objetivo general (Sección~\ref{subsec:objetivo_general}) se considera \textbf{alcanzado}: se trataba de diseñar y desarrollar un sistema de cálculo de rutas seguras para peatones en Madrid, integrando iluminación, vulnerabilidad territorial y accidentalidad mediante enrutamiento sobre grafos ponderados, y el Capítulo~\ref{cap:pruebas} ha demostrado, sobre casos concretos, que el sistema resultante ofrece efectivamente una alternativa medible en seguridad a la ruta rápida convencional. La Tabla~\ref{tab:cumplimiento_oe} detalla el grado de cumplimiento de cada uno de los seis objetivos específicos (Sección~\ref{subsec:objetivos_especificos}).
...
La hipótesis de partida (Sección~\ref{subsec:hipotesis}) afirmaba que es posible calcular rutas peatonales que reduzcan la exposición a factores de riesgo urbano con un incremento asumible de distancia o tiempo de recorrido. Los resultados del Capítulo~\ref{cap:pruebas} \textbf{confirman la hipótesis} dentro del alcance evaluado: un incremento agregado del 3,7\,\% en distancia se corresponde con una reducción del 50,7\,\% en el número total de atropellos y del 13,9\,\% de media en el índice de peligrosidad (Sección~\ref{sec:resultados_comparativos}), una relación coste-beneficio favorable y del mismo orden cualitativo que la documentada por Sohrabi y Lord \cite{sohrabilord2022} para el ámbito vehicular. Esta confirmación está sujeta a las limitaciones de la propia validación, discutidas en la Sección~\ref{sec:limitaciones_validacion} y retomadas a continuación.
...
\textbf{Limitaciones de los datos de entrada.} No existe una fuente pública de criminalidad con desagregación geográfica para Madrid (Sección~\ref{sec:motivacion}), por lo que el sistema aproxima la seguridad ciudadana mediante un índice de vulnerabilidad territorial agregado por distrito (Sección~\ref{subsec:vulnerabilidad_datos}). Se trata de una variable \textit{proxy}, agregada a una granularidad muy superior a la del resto de indicadores (por distrito, frente al segmento de calle individual), y no de una medida directa de criminalidad. Más en general, el sistema depende por completo de portales de datos abiertos externos cuya disponibilidad, formato y granularidad no están garantizados en el tiempo: el Ayuntamiento puede actualizar, reestructurar o retirar cualquiera de estos conjuntos de datos sin previo aviso. Mantener el sistema operativo de forma sostenida exigiría, por tanto, un seguimiento periódico de estas fuentes o, idealmente, algún acuerdo de colaboración con el Ayuntamiento de Madrid que garantizara su continuidad.

\textbf{Limitaciones del modelo de coste.} El índice de peligrosidad es único e independiente de la hora del día (Sección~\ref{sec:vision_general}): no se distingue entre exposición diurna y nocturna, a pesar de que trabajos como el de Levy et al. \cite{levy2020} subrayan la relevancia de esa franja horaria en la percepción del riesgo. Esta simplificación se adoptó por la falta de una segmentación horaria robusta en los datos de accidentalidad disponibles y por la ausencia, ya mencionada, de un indicador de criminalidad real que justificara empíricamente una ponderación temporal.

\textbf{Limitaciones de la validación.} Como se detalló en la Sección~\ref{sec:limitaciones_validacion}, la evaluación del Capítulo~\ref{cap:pruebas} es interna: compara la ruta segura con la rápida usando los mismos indicadores que alimentan la función de coste, sin contraste contra un desenlace real independiente. Además, se apoya en una muestra reducida y deliberadamente no aleatoria de ocho pares origen-destino, y no incluye ninguna prueba de usabilidad o de percepción de seguridad con personas reales sobre el visor web.
...
\begin{enumerate}
    ...
    \item \textbf{Añadir una dimensión temporal al índice de peligrosidad.} Calcular indicadores de iluminación y accidentalidad diferenciados por franja horaria (día/noche, o incluso por hora), y permitir al usuario del visor indicar la hora prevista del trayecto, acercaría el sistema a la propuesta original del proyecto y a enfoques como el de Levy et al.\ \cite{levy2020}. Idealmente, la franja de iluminación debería basarse en los horarios reales de encendido y apagado del alumbrado público que fija el Ayuntamiento de Madrid (variables a lo largo del año según criterios astronómicos de orto y ocaso) en lugar de una división día/noche fija, lo que exigiría localizar esa fuente de horarios y no solo la ubicación de los puntos de luz ya utilizada.
    ...
    \item \textbf{Ampliar la cobertura de pruebas e integrarlas en un flujo de integración continua.} El plan de pruebas actual (Sección~\ref{sec:plan_pruebas}) no cubre de forma individual todos los scripts ETL, y se ejecuta manualmente al no existir un pipeline de integración continua; extender la cobertura a las fuentes de datos restantes y automatizar la ejecución de \texttt{pytest tests/} en cada cambio reforzaría la confianza en el sistema a medida que evolucione.
