...
\begin{itemize}
    \item \texttt{scripts/} — los ocho scripts ETL y de cálculo descritos en la Sección~\ref{sec:metodologia_desarrollo}, más el módulo \texttt{rutas.py}.
...
\begin{itemize}
    ...
    \item \texttt{GET /api/ruta} — recibe las coordenadas de origen y destino como parámetros de consulta, invoca \texttt{calcular\_ruta()} (Sección~\ref{sec:enrutamiento}) una vez por cada criterio (\texttt{rapida} y \texttt{segura}) y devuelve ambos resultados en un único objeto JSON.
    \item \texttt{GET /api/geocode} — recibe un texto de búsqueda y actúa de \textit{proxy} hacia el servicio de geocodificación Nominatim de OpenStreetMap, acotando la búsqueda al término municipal de Madrid mediante los parámetros \texttt{countrycodes} y \texttt{viewbox}.
    \item \texttt{GET /api/geocode/inverso} — recibe unas coordenadas y devuelve la dirección más cercana, también vía Nominatim, usada al hacer clic directamente sobre el mapa.
\end{itemize}

Que el backend actúe como intermediario de Nominatim, en lugar de que el cliente lo consulte directamente, tiene dos motivos: permite fijar desde el servidor una cabecera \texttt{User-Agent} identificativa de la aplicación, tal como recomienda la política de uso de Nominatim, y evita exponer la URL del servicio externo directamente al código JavaScript servido al navegador.
...
\begin{itemize}
    \item Un \textbf{campo de búsqueda} por cada punto (origen y destino), con autocompletado contra \texttt{/api/geocode}: la petición se lanza con un \textit{debounce} de 400\,ms tras dejar de escribir, para no saturar el servicio de geocodificación con una petición por cada pulsación de tecla.
...
Las Figuras~\ref{fig:visor_simple} y~\ref{fig:visor_detallado} muestran el prototipo en funcionamiento sobre el par Callao -- Puerta de Alcalá, uno de los pares origen-destino que se retoman con más detalle en la evaluación del Capítulo~\ref{cap:pruebas}. En el modo simple (Figura~\ref{fig:visor_simple}) se aprecia la ruta recomendada, su distancia y duración, y la insignia cualitativa de seguridad (\textit{Moderada} en este caso, al tratarse de un trayecto por el centro de Madrid). En el modo detallado (Figura~\ref{fig:visor_detallado}) se superponen ambas rutas —discontinua en azul la rápida, continua en verde la segura— junto con la tabla comparativa de indicadores, cuyos valores coinciden con los recogidos más adelante en la Tabla~\ref{tab:resultados_rutas}.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{imagenes/visor_simple.png}
\caption{Visor web, modo simple, para el trayecto Callao -- Puerta de Alcalá.}
