...
Este capítulo describe en detalle el sistema propuesto: su arquitectura general, las fuentes de datos utilizadas, el proceso de extracción y transformación de datos, el modelado de la red viaria, el diseño de la función de coste de seguridad y la implementación del enrutamiento. El sistema se denomina en adelante \textbf{SafeWalk-Madrid}.
...
SafeWalk-Madrid es un sistema de cálculo de rutas seguras para peatones en el entorno urbano de Madrid. Su objetivo es ofrecer al usuario, dado un origen y un destino, una ruta que minimice la exposición a factores de riesgo urbano, como alternativa informada a la ruta más corta convencional.
...
\node[comp, below=1.3cm of fuentes] (etl) {Pipeline ETL\\
    \scriptsize \texttt{scripts/01\_}--\texttt{07\_} (Python, osmnx, GeoPandas)};
...
La Figura~\ref{fig:mapa_peligrosidad} muestra el resultado de aplicar la Ecuación~\ref{eq:coste} a los 524\,292 tramos de la red. El patrón que emerge es coherente con lo que cabría esperar de una ciudad como Madrid: el índice de peligrosidad se concentra sobre todo en el distrito Centro y en los principales ejes de tráfico denso que lo atraviesan, mientras que la mayor parte de la periferia residencial presenta valores bajos. Este mapa anticipa, de forma visual, el patrón cuantitativo que se documenta con más detalle en el Capítulo~\ref{cap:pruebas}: los pares origen-destino que discurren por el centro son precisamente aquellos donde la ruta segura se desvía más de la ruta rápida.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{imagenes/mapa_peligrosidad.png}
\caption{Índice de peligrosidad $p(e)$ por tramo en la red peatonal de Madrid (verde: bajo, rojo: alto). Generado por \texttt{scripts/10\_mapa\_peligrosidad.py}.}
...
\begin{lstlisting}[style=SQL, caption={Consulta pgRouting para el cálculo de la ruta segura.}, label={lst:pgrouting}]
...
\begin{itemize}
    ...
    \item \textbf{Backend}: servidor ligero en Python (Flask) con tres rutas: \texttt{GET /api/ruta}, que recibe las coordenadas de origen y destino, resuelve los nodos más cercanos, lanza las dos consultas de enrutamiento (segura y rápida) contra la base de datos y devuelve las geometrías resultantes en GeoJSON junto con sus estadísticas; y \texttt{GET /api/geocode} y \texttt{GET /api/geocode/inverso}, que actúan como \textit{proxy} del servicio de geocodificación Nominatim de OpenStreetMap, evitando exponer directamente ese servicio externo al cliente.
